...
-- 3. Write a function that attempts to evaluate an expression to normal form using call-by-value. (cbv)

leftInner :: Exp -> Maybe Exp
leftInner e = norm e where
  norm e
    | e == (leftInner' e)    = Nothing
    | otherwise             = Just e
  leftInner' (Cons c) = (Cons c)
  leftInner' (Var x) = Var x
  leftInner' (Abs v e) = Abs v (leftInner' e)
  leftInner' (App (Abs v e) e2) = sub e2 (Var v) e
  leftInner' (App e1 e2) = App (leftInner' e1) e2



-- 4. Write a function that attempts to evaluate an expression to normal form using call-by-name. (cbn)

leftOuter :: Exp -> Maybe Exp
leftOuter e = norm e where
  norm e
    | e == (leftOuter' e)         = Nothing
    | otherwise                   = Just e
  leftOuter' (Cons c)             = Cons c
  leftOuter' (Var x)              = Var x
  leftOuter' (Abs v e)            = Abs v (leftOuter' e)
  leftOuter' (App e1 (Abs v e))   = sub e1 (Var v) e
  leftOuter' (App e1 e2)          = App e1 (leftOuter' e2)
...
cbn e = cbn' e where
  cbn' x
    | leftInner x == Nothing = x
    | otherwise              = cbn' (unwrap . leftInner $ x)
  unwrap (Just e)            = e
...
cbv e = cbv' e where
  cbv' x
    | leftOuter x == Nothing = x
    | otherwise              = cbv' (unwrap . leftOuter $ x)
  unwrap (Just e)            = e
